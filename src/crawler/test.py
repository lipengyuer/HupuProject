...
def getAPage(myurl):  #
    ...
    while True:
        try:
            if count == 1:
                break
            response = runTime.opener.open(myurl,
                                           timeout=10)
...
def  initBloomFilter():
    for line in runTime.mongoCollection.find({}, {'_id':1, 'userName':1}).limit(100):
        #print("布隆过滤器", line)
