...
class genderDetection():
    ...
    def loadFeatureWords(self, path):
        ...
        with open(path, 'r', encoding="utf8") as f:
            lines = f.readlines()
            lines = list(map(lambda x: x.replace("\n", "").split('是')[1][1:], lines))
        words = re.findall("\'.+?\'",lines[1])[0:int(0.5*N_word)] + \
                                           re.findall("\'.+?\'",lines[4])[0:N_word-int(0.5*N_word)]
        words = list(map(lambda x: x.replace('\'', ''), words))
        posTags = re.findall("\'.+?\'", lines[2])[0:int(0.5*N_pos)] + \
                                           re.findall("\'.+?\'", lines[5])[0:N_pos-int(0.5*N_pos)]
        posTags = list(map(lambda x: x.replace('\'', ''), posTags))
        chars = re.findall("\'.+?\'", lines[0])[0:int(0.5*N_char)] + \
                                           re.findall("\'.+?\'", lines[3])[0:N_char-int(0.5*N_char)]
